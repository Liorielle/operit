...
class SupabaseClient:
    ...
    async def get(self, table: str, query: str = "") -> List[Dict]:
        """GET请求"""
    ...
    async def patch(self, table: str, data: Dict, query: str) -> List[Dict]:
        """PATCH请求"""
    ...
    async def delete(self, table: str, query: str) -> Optional[str]:
        """DELETE请求"""
        url = f"{self.url}/rest/v1/{table}?{query}"
        ...
        async with httpx.AsyncClient() as client:
            ...
            response.raise_for_status()
            # Supabase删除操作返回空响应，这里返回状态信息
            return response.headers.get('content-range', '0-0/0')
...
def get_supabase_client() -> SupabaseClient:
    ...
    if _supabase_client is None:
        ...
        _supabase_client = SupabaseClient(SUPABASE_URL, SUPABASE_KEY)
...
async def get_all_categories() -> List[Dict]:
    ...
    return await client.get("categories", "select=id,name&order=name.asc")

async def match_category_by_name(name: str) -> Optional[int]:
    """根据名称匹配分类"""
    client = get_supabase_client()
    try:
        categories = await client.get("categories", f"select=id,name&name=eq.{name}")
        return categories[0]['id'] if categories else None
    except Exception:
        return None

async def get_embedding(text: str) -> List[float]:
    """生成embedding（这里需要根据你的实际情况实现）"""
    # 如果你有embedding API，在这里实现
    # 暂时返回空列表
    return []

async def resolve_model_endpoint(model: str) -> tuple:
    """解析模型端点（保持原有逻辑）"""
    return MEMORY_API_BASE_URL, MEMORY_API_KEY

async def save_memory(content: str, importance: int, source_session: str, 
                     embedding: Optional[str], title: Optional[str], 
                     memory_type: str, created_at: str, 
                     category_id: Optional[int], source: str) -> int:
    """保存记忆"""
    client = get_supabase_client()
    data = {
        'content': content,
    ...
    result = await client.post("memories", data)
...
async def save_calendar_page(date_str: str, page_type: str, sections: List[Dict], 
                            diary: str, keywords: List[str], model_used: str,
                            summary: str = "", digest: str = "") -> int:
    """保存日历页面"""
    client = get_supabase_client()
    data = {
        'date': date_str,
    ...
    result = await client.post("calendar_pages", data)
    return result.get('id') if result else None

async def get_calendar_range(start: str, end: str, page_type: str) -> List[Dict]:
    """获取日历范围"""
    client = get_supabase_client()
    try:
        return await client.get("calendar_pages", f"select=date,type,sections,diary,keywords,summary,digest&date=gte.{start}&date=lte.{end}&type=eq.{page_type}&order=date.asc")
    except Exception:
        return []

async def get_calendar_page(date_str: str, page_type: str) -> Optional[Dict]:
    """获取日历页面"""
    client = get_supabase_client()
    try:
        pages = await client.get("calendar_pages", f"select=date,type,sections,diary,keywords,summary,digest&date=eq.{date_str}&type=eq.{page_type}&limit=1")
...
async def get_config(key: str) -> Optional[str]:
    ...
    client = get_supabase_client()
    try:
        configs = await client.get("configs", f"select=value&key=eq.{key}&limit=1")
        return configs[0]['value'] if configs else None
    except Exception:
        return None
...
async def set_config(key: str, value: str) -> bool:
    ...
    try:
        # 先尝试更新
        existing = await client.get("configs", f"select=id&key=eq.{key}&limit=1")
        if existing:
            await client.patch("configs", {'value': value}, f"key=eq.{key}")
        else:
            await client.post("configs", {'key': key, 'value': value})
        return True
    except Exception:
        return False
...
async def _run_daily_digest_impl(date_str: str, now_cst, model_override: str = None, prompt_override: str = None):
    ...
    try:
        client = get_supabase_client()
        # 使用日期范围查询碎片记忆
        start_time = f"{date_str}T00:00:00+08:00"
        end_time = f"{date_str}T23:59:59+08:00"
        fragments = await client.get("memories", f"select=id,title,content,importance,created_at&memory_type=eq.fragment&created_at=gte.{start_time}&created_at=lt.{end_time}&order=created_at.asc")
    except Exception as e:
        print(f"   ⚠️ 查询碎片记忆失败: {e}")
        return {"date": date_str, "fragments": 0, "digests": 0, "error": str(e)}
    ...
    if len(fragments) <= 2:
        try:
            ...
            for fid in fragment_ids:
                await client.patch("memories", {'memory_type': 'daily_digest'}, f"id=eq.{fid}")
        except Exception as e:
            print(f"   ⚠️ 更新记忆类型失败: {e}")
    ...
    for d in digests:
        ...
        try:
            await save_memory(
                ...
                "daily_digest", f"{date_str}T00:00:00+08:00", cat_id, "ai_digest"
            )
            saved_count += 1
    ...
    try:
        fragment_ids = [f["id"] for f in fragments]
        for fid in fragment_ids:
            await client.patch("memories", {'memory_type': 'digested'}, f"id=eq.{fid}")
...
async def update_user_profile(digest_text: str = None, model_override: str = None, prompt_override: str = None):
    ...
    if not digest_text:
        # 读最近的日页面
        page = await get_calendar_page("", "day")  # 获取最新的日页面
        if not page:
        ...
        if page.get("diary"):
            parts.append(f"AI 的话：{page['diary']}")
        digest_text = "\n\n".join(parts)
...
async def cleanup_expired_fragments():
    ...
    try:
        ...
        # 注意：Supabase REST API 不支持复杂的子查询，需要分步处理
        
        # 先获取需要检查的碎片
        old_date = (datetime.now(TZ_CST) - timedelta(days=7)).isoformat()
        fragments_to_check = await client.get("memories", f"select=id,created_at&memory_type=eq.fragment&importance=lt.8&created_at=lt.{old_date}")
        
        count1 = 0
        for f in fragments_to_check:
            # 检查是否有对应的日页面
            frag_date = f["created_at"].split("T")[0]
            day_page = await get_calendar_page(frag_date, "day")
            if day_page:
                # 检查是否已处理过Dream（假设有dream_processed_at字段）
                # 这里简化处理，直接删除
                await client.delete("memories", f"id=eq.{f['id']}")
                count1 += 1

        # 删除重要碎片（importance >= 8，超过30天）
        old_date_imp = (datetime.now(TZ_CST) - timedelta(days=30)).isoformat()
        important_fragments = await client.get("memories", f"select=id,created_at&memory_type=eq.fragment&importance=gte.8&created_at=lt.{old_date_imp}")
        ...
        dream_deleted = await client.get("memories", f"select=id&memory_type=eq.dream_deleted&created_at=lt.{old_date_imp}")
        count3 = 0
        for f in dream_deleted:
            await client.delete("memories", f"id=eq.{f['id']}")
            count3 += 1

        # 清理已失效且过期的碎片（valid_until 不为 NULL 且超过 30 天）
        invalid_fragments = await client.get("memories", f"select=id,valid_until&memory_type=eq.fragment&valid_until=lt.{old_date_imp}")
        count4 = 0
        for f in invalid_fragments:
            await client.delete("memories", f"id=eq.{f['id']}")
            count4 += 1
...
async def generate_day_page(target_date: str = None, model_override: str = None):
    ...
    start_time = f"{date_str}T00:00:00+08:00"
    end_time = f"{date_str}T23:59:59+08:00"
    fragments = await client.get("memories", f"select=title,content&memory_type=eq.fragment&created_at=gte.{start_time}&created_at=lt.{end_time}&order=created_at.asc")
...
async def test_supabase_connection():
    ...
    try:
        client = get_supabase_client()
        # 测试获取分类
        categories = await client.get("categories", "select=id,name&limit=1")
        print(f"✅ Supabase连接成功，找到分类: {categories}")
        return True
    except Exception as e:
