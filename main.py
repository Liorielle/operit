...
async def ghost_ping_keep_alive():
    ...
    # 北京时间 14点到23点，或者 0点到凌晨3点(不到4点) 生效
    ...
    ghost_payload["messages"].append(
        {"role": "user", "content": "System Keep-alive: reply with a single period (.)"}
    )
        ...
        if "messages" in target_dict:
            ...
            if total_msgs >= TRIGGER_MSGS:
                ...
                if total_msgs > FROZEN_MSGS:
                    bp3 = raw_chat_history[:FROZEN_MSGS]
                    bp4 = raw_chat_history[FROZEN_MSGS:]
                else:
                    # 还没攒够冻结区，全部放进 BP3
                    bp3 = raw_chat_history
                    bp4 = []
            ...
            if bp3:
                ...
                last_bp3["content"] = [
                    {
                        "type": "text",
                        "text": str(last_bp3.get("content", "")),
                        "cache_control": {"type": "ephemeral", "ttl": "1h"},
                    }
                ]

            # --- BP4（滑动白嫖位，最后一条 User 打动态断点）---
            if bp4:
                for msg in reversed(bp4):
            ...
            # 如果有 cold_start_context，作为段5（不缓存）放在末尾之前
            if cold_start_context:
                final_messages.append(
                    {
                        "role": "user",
                        "content": [{"type": "text", "text": cold_start_context}],
                    }
                )
